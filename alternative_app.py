    ...
    if not all([client_name, project_name, main_deliverable]):
        ...
        with output_container:
            with st.spinner("⚙️ Generating your bulletproof contract..."):
                ...
                st.markdown("## 📄 Your Generated Scope of Work")
                st.markdown("---")
                st.text_area("Contract Text", contract_text, height=500, key="contract_display")
...
if st.session_state.show_pdf_section and st.session_state.generated_contract:
    ...
    if st.session_state.pdf_ready:
        ...
        with col2:
            st.info("💡 **Pro Tip:** Send this PDF to your client BEFORE starting work. It sets clear expectations and prevents scope creep.")
    else:
        st.warning("PDF generation failed. You can still copy the text above.")

# Add some helpful tips
with st.expander("💡 Best Practices for Using Your Contract"):
    st.markdown("""
    **1. Send Early:** Send the Scope of Work BEFORE starting any work
    **2. Get Signatures:** Have both parties sign (electronic signature is fine)
    **3. Refer Often:** Refer back to the "EXCLUSIONS" section when clients ask for extras
    **4. Change Requests:** Use the Change Request Process for any additions
    **5. Keep Records:** Store signed copies in a secure location
    
    **Common Scope Creep Traps to Watch For:**
    - "While you're at it, can you also..."
    - "This should be quick/easy for you"
